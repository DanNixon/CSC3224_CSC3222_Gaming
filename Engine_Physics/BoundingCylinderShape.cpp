...
namespace Physics
{
  ...
  void BoundingCylinderShape::updateDimensionFromSceneTree(SceneObject *object, size_t maxDepth, size_t level)
  {
    ...
    if (level == 0)
    {
      Vector3 boxDims = m_box.size() * 0.5f;
      ...
      // Apply scaling based on local model matrix
      m_scaling = object->modelMatrix().scalingVector();
      setLocalScaling(MathsConversions::ToBullet(m_scaling));
    }
  }
}
}
