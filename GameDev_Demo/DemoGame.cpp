...
int DemoGame::gameStartup()
{
  ...
  m_uiShader->addShader(
    new FragmentShader("../resources/shader/frag_col.glsl"));
  m_uiShader->link();

  m_ui = new Scene(new SceneObject("root"),
    Matrix4::BuildViewMatrix(Vector3(0, 0, 0), Vector3(0, 0, -1000)),
    Matrix4::Orthographic(0.0f, -1.0f, 10.0f, -10.0f, 10.0f, -10.0f));

  RenderableObject * leftStickArea = new RenderableObject("left_stick_area", Mesh::GenerateRect2D(Vector2(2.0f, 2.0f)), m_uiShader);
  ...
  RenderableObject * rightStickArea = new RenderableObject("right_stick_area", Mesh::GenerateRect2D(Vector2(2.0f, 2.0f)), m_uiShader);
...
void DemoGame::gameLoop(Uint8 id, float dtMilliSec)
{
  if (id == m_graphicsLoop)
  {
    ...
    m_leftStick->setModelMatrix(Matrix4::Translation(Vector3(m_simControls->analog(A_YAW), m_simControls->analog(A_THROT), 0.1f)));
    m_rightStick->setModelMatrix(Matrix4::Translation(Vector3(m_simControls->analog(A_ROLL), m_simControls->analog(A_PITCH), 0.1f)));
